function Initialize()
	Oldtime = {SKIN:GetMeasure("MeasureTime"):GetValue(),SKIN:GetMeasure("MeasureTime"):GetValue(),SKIN:GetMeasure("MeasureTime"):GetValue(),SKIN:GetMeasure("MeasureTime"):GetValue()}
	...
	scale = SKIN:GetVariable('PosScale')
	offset = 0
	...
	for i=1,6 do
		...
		for j=1,2 do
			...
			for k=1,2 do
				...
				SKIN:GetMeter('MeterMid'..i..j..k):SetX(+((j-1)*50)+offset)
			end
		end
	end
end
...
function Update()
	Time[1] = math.floor((SKIN:GetMeasure("MeasureTime"):GetValue())/36000)%10
	Time[2] = math.floor((SKIN:GetMeasure("MeasureTime"):GetValue())/3600)%10
	Time[3] = math.floor((SKIN:GetMeasure("MeasureTime"):GetValue())/600)%10
